...
class HrQrDisplayScreen extends StatefulWidget {
  const HrQrDisplayScreen({super.key});

  @override
  State<HrQrDisplayScreen> createState() => _HrQrDisplayScreenState();
}

class _HrQrDisplayScreenState extends State<HrQrDisplayScreen> {
  ...
  Future<void> _loadSession({bool refresh = false}) async {
    ...
    try {
      final session = await AppServices.attendanceRepository
          .createAttendanceQrSession(role: AppUserRole.hr, refresh: refresh);
      if (!mounted) {
        return;
      }
        ...
        child: SafeArea(
          ...
          child: _isLoading
              ? const AppLoadingState(
                  ...
                  message: 'نحضّر جلسة QR الحالية الخاصة بالموارد البشرية.',
                )
              : _errorMessage != null
              ? AppErrorState(
                  title: 'حدث خطأ',
                  ...
                  child: Center(
                    child: ConstrainedBox(
                      ...
                      child: Column(
                        ...
                        children: [
                          _QrHero(
                            title: 'باركود حضور ثابت للموارد البشرية',
                            subtitle:
                                'يبقى هذا الرمز ثابتًا إلى أن يتم تغييره يدويًا من نفس الصفحة. عند إصدار رمز جديد يصبح الرمز السابق غير صالح مباشرة.',
                            token: session.token,
                          ),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: const Text(
                                    'يمكن للموظف مسح هذا الباركود في أي وقت، وسيبقى نفس الرمز معتمدًا حتى يتم تغيير الباركود يدويًا من قبل الموارد البشرية.',
                                  ),
                                ),
                                const SizedBox(height: 18),
                                ElevatedButton.icon(
                            ...
                            child: const Column(
                              ...
                              children: [
                                _GuidePoint(
                                  ...
                                  text:
                                      'الرمز لا يتغير تلقائيًا بعد الآن، بل يتم تحديثه فقط عند الضغط على زر تغيير الباركود.',
                                ),
                                SizedBox(height: 12),
                                _GuidePoint(
                                  icon: Icons.verified_user_outlined,
                                  text:
                                      'يبقى نفس الرمز معتمدًا للمسح حتى لو بقيت الصفحة مفتوحة لفترة طويلة دون أي تغيير.',
                                ),
                                SizedBox(height: 12),
                                _GuidePoint(
                                  icon: Icons.qr_code_scanner_rounded,
                                  text:
                                      'بعد إصدار رمز جديد يدويًا، يعتمد النظام الرمز الجديد فقط ويتم رفض أي محاولة باستخدام الرمز السابق.',
                                ),
                              ],
                            ),
                          ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          ...
          colors: [Color(0xFF262A0F), Color(0xFF575C10), Color(0xFF95C51C)],
        ),
        borderRadius: BorderRadius.circular(30),
      ),
      child: Column(
