...
import 'manager_employee_details_screen.dart';

class ManagerLeaveRequestDetailsScreen extends StatefulWidget {
  const ManagerLeaveRequestDetailsScreen({super.key, required this.request});

  final ManagerLeaveRequest request;

  @override
  State<ManagerLeaveRequestDetailsScreen> createState() =>
      _ManagerLeaveRequestDetailsScreenState();
}

class _ManagerLeaveRequestDetailsScreenState
    extends State<ManagerLeaveRequestDetailsScreen> {
  late ManagerLeaveRequest _request;
  final _decisionNoteController = TextEditingController();
  bool _isSubmittingDecision = false;
  bool _isOpeningEmployeeFile = false;

  bool get _canDecide =>
      _request.status == ManagerLeaveWorkflowStatus.pendingReview;
  ...
  Future<void> _submitDecision({required bool approve}) async {
    ...
    try {
      final updatedRequest = await AppServices.leaveRepository
      ...
      _showDecisionSnackBar(
        approve
            ? 'تمت الموافقة على الطلب وتحويله إلى الموارد البشرية.'
            : 'تم رفض الطلب وإيقاف مسار الموافقة.',
      );
    } catch (error) {
      if (!mounted) {
        return;
  ...
  Future<void> _openEmployeeFile() async {
    ...
    try {
      ...
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => ManagerEmployeeDetailsScreen(profile: profile),
        ),
      );
    } catch (error) {
      if (!mounted) {
                        ...
                        children: [
                          Expanded(
                            child: _DetailsSection(
                              title: 'بيانات الموظف والطلب',
                              children: [
                                _InfoRow(
                                  label: 'اسم الموظف',
                                  value:
                                      '${_request.employeeName} (${_request.employeeCode})',
                                ),
                                _InfoRow(
                                  label: 'القسم',
                                ...
                                _InfoRow(
                                  label: 'آخر حضور',
                                  value: _request.lastAttendanceLabel,
                                ),
                                _InfoRow(
                                  label: 'ملاحظة الموظف',
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.stretch,
                              children: [
                                _ApprovalStatusCard(request: _request),
                                const SizedBox(height: 16),
                                _DecisionCard(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'راجع تفاصيل الطلب، تأكد من الرصيد وسجل الموظف، ثم اتخذ القرار المناسب بالموافقة أو الرفض.',
...
class _ApprovalStatusCard extends StatelessWidget {
  const _ApprovalStatusCard({required this.request});
      ...
      child: Column(
        ...
        children: [
          ...
          _StatusBlock(
            ...
            color: request.status == ManagerLeaveWorkflowStatus.rejected
          ...
          _StatusBlock(
            ...
            statusLabel:
                ...
                : 'توقفت المعالجة',
            note: request.hrNote,
            color: request.status == ManagerLeaveWorkflowStatus.fullyApproved
                ? const Color(0xFF0F766E)
                : request.status == ManagerLeaveWorkflowStatus.rejected
                ? const Color(0xFFDC2626)
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text('قرار المدير', style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 12),
          Text(
            canDecide
                ? 'يمكنك إضافة ملاحظة داخلية قبل الموافقة أو الرفض.'
                : 'تم اتخاذ القرار على هذا الطلب بالفعل في هذه الواجهة التجريبية.',
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'أضف سبب الموافقة أو الرفض إن لزم',
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(isSubmitting ? '...' : 'Reject'),
                ),
              ),
              ...
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: Text(isSubmitting ? '...' : 'Approve'),
                ),
              ),
            ],
          ),
